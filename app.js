...
try{
 ...
 console.error("Primoria failed to render:",err);
 const main=document.querySelector("#main");
 if(main){
  main.innerHTML=`<section class="hero"><div class="eyebrow">Primoria · Recovery</div><h2>The application encountered a loading error.</h2><p>Please clear the old service worker/cache, then reload. If this message remains, one or more deployment files are missing or mismatched.</p><pre style="white-space:pre-wrap;overflow:auto">${esc(err?.message||String(err))}</pre></section>`;
 }
}
if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js").catch(e=>console.warn("Service worker registration failed.",e)))}
